...
Only convert to true if the bounding box is at the center
of the 224x224 image.
"""
...
def a_bounding_box_is_in_center(box_list, roi_point, roi_size):
    """Check if any of the bounding boxes are in the ROI center.
    ...
    for box in box_list:
        ...
        if box.y2 < box.y1:
            ...
            box_topedge = box.y1
        if (
            box_bottomedge > roi_point[1] and
            box_topedge < roi_point[1] and
            box_leftedge < roi_point[0] and
            box_rightedge > roi_point[0]
           ):
            return True
    return False
...
ap.add_argument('-o', '--output_folder',
                required=True,
                help='Folder to put processed training images')
...
for scan in scanpaths:
    ...
    for imagename in imagenames:
        ...
        for point in pointlist:
            ...
            if not a_bounding_box_is_in_center(boxes, point, size):
                # save to 'False'
