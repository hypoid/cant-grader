...
for filename in filenames:
    ...
    list_of_boxes = pickle.load(open(filename[:-4]+ftype, 'rb'))
    ...
    while True:
        ...
        if k == 13 and Clicked is not True:  # If 'Enter' is pressed
            with open(filename[:-4]+ftype+'.checked',
                      'wb') as file_to_dump_into:
                pickle.dump(' ', file_to_dump_into)
                print(filename[:-4]+ftype+'.checked written')
            if padded is True:
                for box in list_of_boxes:
                    box.ymin -= pad_height
                    box.ymax -= pad_height
            with open(filename[:-4]+ftype, 'wb') as file_to_dump_into:
                ...
                print(filename[:-4]+ftype+' re-written')
            break
